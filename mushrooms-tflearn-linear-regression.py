'''
Mushrooms TF-Learn linerar regression
...
def initial_population(training_input_file_name, training_data_percentage, save_data_file_header):
    ...
    # funzione che converte il valore in ascii da applicare a tutti i dati
    def toOrd(x):
        return ord(x)

    toOrdVect = np.vectorize(toOrd)    
    
    dataframe = pandas.read_csv(training_input_file_name, delim_whitespace=False, header=0)
    dataset = dataframe.values
    
    # applica la funzione a tutti i dati
    dataset = toOrdVect(dataset)    
    ...
    print('-- training_data -- only 10')
    print(tabulate(training_data[0:10,:]))
    
    print('-- test_data -- only 10')
    print(tabulate(test_data[0:10,:]))
    
    #print('-- evaluate_data -- only 10')
    #print(tabulate(evaluate_data[0:10,:]))
...
# @Gooey()
def main():
    parser = argparse.ArgumentParser(description='Script to gereric TF-Learn Linear Regression')
    parser.add_argument("--model_file_name", 
                                default='mushroom.tflearn',
    ...
    parser.add_argument("--net_input_size", 
                                ...
                                type=int,
                                # default="mobile_ssd/ssd_mobilenet_v1_coco.pbtxt",
                                help='network input size - number of features')                                
    ...
    parser.add_argument("--training_data_file_name", 
                                default="mushrooms-training-data.npy",
